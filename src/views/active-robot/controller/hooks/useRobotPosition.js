...
import { initGlobalResetSmoothing, updateGlobalResetSmoothing, startSmoothReset, getCurrentSmoothedValues } from '@utils/globalResetSmoothing';
...
export function useRobotPosition(isActive) {
  ...
  // Initialize global reset smoothing system (persists across unmounts)
  // Use sendCommandRef to always have the latest version
  useEffect(() => {
    const globalSendCommand = (headPose, antennas, bodyYaw) => {
      // Use the latest sendCommand via ref
      sendCommandRef.current(headPose, antennas, bodyYaw);
    };
    initGlobalResetSmoothing(globalSendCommand, isActive);
    return () => {
      // Update when unmounting (but keep the system alive)
      updateGlobalResetSmoothing(false);
    };
  }, [isActive]);

  // Update global reset smoothing when isActive changes
  useEffect(() => {
  ...
  // Smooth reset: animates progressively to zero instead of instant reset
  // Uses global reset system that continues even after component unmounts
  const resetAllValuesSmooth = useCallback(() => {
    // Get current smoothed values from local smoothing manager
    const currentSmoothed = targetSmoothingRef.current.getCurrentValues();
    
    // Update local values to reflect the target (for UI)
    const zeroTargets = {
      headPose: { x: 0, y: 0, z: 0, pitch: 0, yaw: 0, roll: 0 },
      bodyYaw: 0,
      antennas: [0, 0],
    ...
    // Also update local smoothing manager (for when component is still mounted)
