...
export interface UseInternetHealthcheckOptions {
  interval?: number;
  timeout?: number;
  endpoint?: string;
}

export interface UseInternetHealthcheckResult {
...
export function useInternetHealthcheck({
...
}: UseInternetHealthcheckOptions = {}): UseInternetHealthcheckResult {
  ...
  const [hasChecked, setHasChecked] = useState<boolean>(false); // Track if we've had at least one real check
  const hasPerformedFirstCheckRef = useRef<boolean>(false); // Track if we've performed at least one check attempt
  const [isChecking, setIsChecking] = useState<boolean>(false);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const abortControllerRef = useRef<AbortController | null>(null);
  const consecutiveFailuresRef = useRef<number>(0); // Track consecutive failures to avoid false negatives
  ...
  const performHealthcheck = useCallback(async () => {
    ...
    try {
      ...
      if (err.name !== 'AbortError') {
        console.warn('⚠️ Internet healthcheck error:', err.name, err.message);
