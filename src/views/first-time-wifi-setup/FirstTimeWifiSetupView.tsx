...
import type { Step4Status } from './steps/Step4Reconnecting';
...
const HOTSPOT_HOSTS: readonly string[] = [
...
export default function FirstTimeWifiSetupView(): React.ReactElement {
  ...
  void isCheckingDaemon;
  const daemonCheckInterval = useRef<IntervalId | null>(null);

  // Step 4: Robot discovery on local network
  const {
    wifiRobot,
    isScanning: isDiscoveryScanning,
    refresh: _refreshDiscovery,
  } = useRobotDiscovery();
  ...
  const checkDaemonReachable = useCallback(async (): Promise<boolean> => {
    setIsCheckingDaemon(true);

    // Try all hosts in parallel
    const checkHost = async (host: string): Promise<string | null> => {
      try {
  ...
  const handleWifiConfigured = useCallback((ssid: string) => {
  ...
  const [reconnectStatus, setReconnectStatus] = useState<Step4Status>('waiting');
  const [foundHost, setFoundHost] = useState<string | null>(null);
  void foundHost;
  const [wifiConfigKey, setWifiConfigKey] = useState<number>(0); // Key to force remount of WiFiConfiguration
  const [isRetryAfterFail, setIsRetryAfterFail] = useState<boolean>(false); // Track if we're retrying after a failed connection

  // Hosts to check for Reachy on normal network (not hotspot IPs)
  const NORMAL_NETWORK_HOSTS: readonly string[] = ['reachy-mini.local', 'reachy-mini.home'];
  void NORMAL_NETWORK_HOSTS;
  ...
  useEffect(() => {
    ...
    const NORMAL_HOSTS = ['reachy-mini.local', 'reachy-mini.home'];

    const checkNormalNetwork = async (): Promise<boolean> => {
      if (isCancelled) return false;

      // Only check NORMAL network hosts (not hotspot)
      for (const host of NORMAL_HOSTS) {
        if (isCancelled) return false;
